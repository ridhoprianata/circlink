...
class FriendSearchBody extends StatefulWidget {
  const FriendSearchBody({Key? key}) : super(key: key);

  @override
  _FriendSearchBodyState createState() => _FriendSearchBodyState();
}

class _FriendSearchBodyState extends State<FriendSearchBody> {
  List<bool> isFriends = List.generate(100, (index) => false);
  var followedEmails = [];
  ...
  Future getRandomUsers() async {
    if (followedEmails.isEmpty) {
      ...
      await FirebaseFirestore.instance
          .collection('users')
          .where('email', whereNotIn: followedEmails)
          .get()
          .then(
            (value) => {
              value.docs.forEach(
